...
const OKLCH_PATTERN = /^oklch\(([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)\)$/;
      ...
      .map(([name, color]) => {
        ...
        return {
          ...
          colors: color.map((color) => {
            const rgb = color.rgb.replace(RGB_PATTERN, "$1 $2 $3");
            ...
            return {
              ...
              rgb,
              hsl: color.hsl.replace(HSL_PATTERN, "$1 $2 $3"),
              oklch: `oklch(${color.oklch.replace(OKLCH_PATTERN, "$1 $2 $3")})`,
