...
function printHelp() {
  console.log(`
Usage: node apps/web/scripts/index-registry-embeddings.mjs [options]
...
async function resolveIndexPath(explicitPath) {
  const candidates = [
    explicitPath,
...
async function withRetry(task, label, maxRetries, baseDelayMs) {
  let attempt = 0;
  while (true) {
    try {
      return await task();
